<nav x-data="{ open: false, mobileOpen: false }" class="hidden md:flex bg-white border-r-4 border-slate-900 h-screen w-64 shadow-[8px_0px_0px_0px_rgba(30,41,59,1)] flex-col gap-4 p-6 fixed left-0 top-0 overflow-y-auto z-40">
    ...
    <div class="flex items-center gap-0 mb-8">
        ...
        <div>
            ...
            <p class="font-technical-xs text-technical-xs text-slate-500 uppercase tracking-widest">Blueprint Mode</p>
        </div>
    </div>
    ...
    <div class="flex-grow flex flex-col gap-2">
        ...
        <a href="{{ route('projects.index') }}"
           class="flex items-center gap-3 px-4 py-3 border-2 font-technical-sm text-technical-sm transition-all
                  {{ request()->routeIs('projects.*') || request()->routeIs('components.*')
                      ? 'bg-blue-500 text-white border-slate-900 shadow-[4px_4px_0px_0px_rgba(30,41,59,1)] -translate-x-[2px]'
                      : 'text-slate-700 border-transparent hover:border-slate-900 hover:translate-x-1 hover:bg-surface-container-low' }}">
            <span class="material-symbols-outlined" style="{{ request()->routeIs('projects.*') || request()->routeIs('components.*') ? 'font-variation-settings: \'FILL\' 1;' : '' }}">folder_open</span>
            <span>Projects</span>
        </a>

        <!-- Component Library -->
        <a href="{{ route('component-library') }}"
           class="flex items-center gap-3 px-4 py-3 border-2 font-technical-sm text-technical-sm transition-all
                  {{ request()->routeIs('component-library')
                      ? 'bg-blue-500 text-white border-slate-900 shadow-[4px_4px_0px_0px_rgba(30,41,59,1)] -translate-x-[2px]'
                      : 'text-slate-700 border-transparent hover:border-slate-900 hover:translate-x-1 hover:bg-surface-container-low' }}">
            <span class="material-symbols-outlined" style="{{ request()->routeIs('component-library') ? 'font-variation-settings: \'FILL\' 1;' : '' }}">grid_view</span>
    ...
    <div class="mt-auto flex flex-col gap-3 border-t-2 border-slate-200 pt-4">
        <div class="flex items-center gap-3 px-2">
            <div class="w-8 h-8 bg-primary-fixed border-2 border-slate-900 pixel-border flex items-center justify-center flex-shrink-0">
                <span class="material-symbols-outlined text-[16px] text-on-primary-fixed">person</span>
            </div>
            <div class="min-w-0">
...
<div class="md:hidden bg-white border-b-4 border-slate-900 px-4 py-3 flex items-center justify-between shadow-[0px_4px_0px_0px_rgba(30,41,59,1)] sticky top-0 z-40 w-full" x-data="{ mobileOpen: false }">
    ...
    <div x-show="mobileOpen" x-cloak
        ...
        <a href="{{ route('projects.index') }}" class="flex items-center gap-3 px-6 py-4 border-b-2 border-slate-100 font-technical-sm text-technical-sm text-slate-700 hover:bg-surface-container-low">
            <span class="material-symbols-outlined text-[20px]">folder_open</span> Projects
        </a>
        <a href="{{ route('component-library') }}" class="flex items-center gap-3 px-6 py-4 border-b-2 border-slate-100 font-technical-sm text-technical-sm text-slate-700 hover:bg-surface-container-low">
            <span class="material-symbols-outlined text-[20px]">grid_view</span> Component Library
        </a>
